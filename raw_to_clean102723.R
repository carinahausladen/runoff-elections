...
# check to make sure that the renaming worked
...
# check to make sure that the sessionids match across the clean dataset (provided by authors) and out compilation of the raw data
identical(
...
#####################
# This is the primary section that we updated in response to the authors' comments

    # per the authors' response to our initial replication report, "in order to correctly clean the raw files, is to replace the values of the socio-demographic characteristics of subjects 1 in period 1 by missing values before filling the cells"
    # those variables are: gender, age, year, risk, trust, experiments, politicosity
    sociodem <- c("gender", "age", "year", "risk", "trust", "experiments", "politicosity")
    
    subjects_updated <- subjects %>%
        mutate(across(all_of(sociodem), ~ifelse(subject == 1 & period == 1, NA, .)))
    
######################
# 3. Creating a dataset with both subject and globals data

both_wide <- subjects_updated %>%
